...
## 8 Hot Spots and Cold Spots Analysis (HCSA)

Hot spot and cold spot analysis is a technique used in spatial statistics to identify statistically significant clusters of high values (hot spots) and low values (cold spots) of a geographically referenced attribute, such as GDPPC. It reveals areas where values are clustered more densely than would be expected by random chance.

A hot spot is an area where high values are concentrated and surrounded by other high values, relative to the rest of the study area. It has been used generically across disciplines to describe a region or value that is higher relative to its surroundings (Lepers et al 2005, Aben et al 2012, Isobe et al 2015). For example, a hot spot analysis might reveal clusters of high GDPPC in a province, indicating areas with significantly higher GDPPC than other parts of the province.

A cold spot is the reverse, an area where low values are concentrated and surrounded by other low values. An example could be clusters of low GDPPC, indicating areas where GDPPC is consistently low.

HCSA uses statistical methods, such as the Getis-Ord Gi*, to test whether observed clustering is statistically significant or could plausibly have arisen by chance.
...
### 8.1 Deriving Distance Weight Matrix

Spatial autocorrelation, as computed in the previous sections, considered counties that share a border. For Getis-Ord Gi*, neighbours are defined by distance. 

Two types of distance-based proximity matrix are used in this exercise: 

- Fixed distance weights
- Adaptive distance weights

#### 8.1.1 Computing fixed distance weights

This section computes the fixed distance weight matrix, using `st_dist_band()` and `st_weights()` of the **sfdep** package.

The upper limit for the distance band is determined first:
...
The above output shows that the largest first-nearest-neighbour distance is 60.80 km, so using this as the upper threshold ensures that every county has at least one neighbour.

The fixed distance weight matrix is then computed using the above critical threshold value.
...
::: callout-tip
For Gi*, `include_self()` must be used to ensure that $w_{ii}(d) \neq 0$.
:::

#### 8.1.2 Computing adaptive distance weights

Under the fixed distance weight matrix, densely settled areas, usually urban counties, tend to have more neighbours, while less densely settled, rural counties tend to have fewer. Having more neighbours smooths the neighbour relationship across a wider area.

The number of neighbours can instead be controlled directly using k-nearest neighbours, either accepting asymmetric neighbours or imposing symmetry, as shown in the code chunk below.

This section computes the adaptive distance weight matrix, using `st_knn()` and `st_weights()` of the **sfdep** package.
...
### 8.2 Computing local Gi*

Local Gi* is a Getis-Ord statistic used to identify statistically significant hot spots and cold spots of high or low values in a dataset. It compares the sum of a variable within a specified neighbourhood, including the location itself, against the global mean, producing a z-score and p-value for each location that indicate whether the observed pattern is unlikely to be due to chance.

This section computes Gi* using `local_gstar_perm() `of the **sfdep** package.
...
### 8.3 Preparing and Visualising HCSA Map

#### 8.3.1 Mapping Gi* with Fixed Distance Weights

Thematic mapping functions of the **tmap** package are used to prepare a choropleth map showing the distribution of Gi* values.
...
The Gi* map shows the same broad pattern identified by the LISA cluster map: a hot spot in the north-east, reaching Gi* values of 5.0 to 6.0, and a band of negative Gi* values spanning the west of the province, from south-west to north-west, with the most negative values around -2.0 to -1.1. The rest of the province shows weaker positive and negative Gi* values.
:::

#### 8.3.2 Mapping Local Gi* p-values

The Gi* map shows evidence of both positive and negative values, but it does not indicate which of these are statistically significant. The p-values for each county are mapped below for that purpose.
...
Three groups of counties show low p-values. The north-east cluster, the confirmed hot spot from the Gi* map, is significant here, mostly in the 0.010 to 0.049 class. The south-west and north-west clusters both fall within the broader cold region which is visible in the Gi* value map, and their significance confirms that at least part of that western spread of negative Gi* values reflects genuine local clustering.
:::

#### 8.3.3 Mapping Gi* values and p-values

The Gi* values map and its p-values map can be plotted side by side for direct comparison.
...
By default, the HCSA classes are stored in the `cluster` field of the `HCSA_fdw` sf data frame. A new field, `HCSA_cluster`, is derived from `cluster` by using the code chunk below.
...
::: callout-note
A new class, `Insignificant`, is added for counties with p_sim > 0.05.
:::

The HCSA map is then plotted using the code chunk below.
...
The HCSA map confirms three significant clusters: a hot spot in the north-east, matching the High-High cluster from the LISA map, and two separate cold spots, one in the south-west and one in the north-west. The LISA cluster map earlier identified only the south-west as a significant Low-Low cluster. The north-west cold spot found reflects a difference between the two methods: Gi* defines neighbours by distance, while LISA defines them by contiguity. The rest of the province shows no significant local clustering by either measure.
:::

The local Gi* values map and the HCSA map can be plotted side by side, to compare the raw Gi* values against the counties confirmed as significant hot spots or cold spots.
